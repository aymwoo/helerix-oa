...
} from "./LessonPlannerAgent";
import {
  PerformanceAnalyzerAgent,
  getPerformanceAnalyzerAgent,
} from "./PerformanceAnalyzerAgent";
import { AIProviderDatabase, AgentUsageDatabase } from "../db";
...
class AgentRegistryImpl {
  ...
  private usageStats: Map<AgentId, { count: number; lastUsed?: number }> =
    new Map();
   ...
   * Record agent usage
   */
  async recordUsage(id: AgentId): Promise<void> {
    const stats = this.usageStats.get(id) || { count: 0 };
    stats.count += 1;
    stats.lastUsed = Date.now();
    this.usageStats.set(id, stats);
    try {
      await AgentUsageDatabase.recordUsage(id);
    } catch (e) {
      console.error("Failed to record usage in DB", e);
    }
  }
  ...
  async loadProviders(): Promise<ProviderConfig[]> {
    const geminiDefault: ProviderConfig = {
      id: "gemini",
      name: "Google Gemini",
      type: "gemini",
  ...
  private async loadUsageStats(): Promise<void> {
    try {
      const data = await AgentUsageDatabase.getStats();
      ...
      console.error("Failed to load usage stats from DB", e);
    }
  }
}
